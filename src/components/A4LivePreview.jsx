...
import { sumScores } from "@/services/scoringUtils";
        ...
        <tr>
          ...
          <td className="gdt-col-right">
            ...
            <p className="gdt-exam-sub">
              Môn: {meta?.subject || "Toán"} &nbsp;-&nbsp; Lớp {meta?.grade || "..."}
            </p>
...
function StudentInfoLine({ meta }) {
  return (
    <p className="gdt-student-line">
      Họ và tên: <span className="gdt-dots">............................................</span>
      &nbsp;&nbsp;Lớp: <span className="gdt-dots">{meta?.className || "........"}</span>
      &nbsp;&nbsp;Trường: <span className="gdt-dots">{meta?.schoolName || "............................................"}</span>
    </p>
  );
}
    ...
    <div className="exam-question">
      <p>
        <span className="q-index">
          Câu {index + 1}{question.score != null ? ` (${question.score}đ)` : ""}:
        </span>{" "}
        <RichMathText text={question.content} />
      </p>
...
function RubricPage({ teacherRubric, questions }) {
  ...
  const idToScore = new Map(questions.map((q) => [q.id, q.score]));

  return (
    <div className="a4-page page-break-before">
      <h2 className="text-center text-base font-bold uppercase">Đáp án &amp; Thang điểm (Rubric)</h2>
      ...
      {teacherRubric.map((r, i) => (
        <div className="exam-question" key={r.questionId || i}>
          <p>
            <span className="q-index">
              Câu {idToIndex.get(r.questionId) ?? "?"}
              {idToScore.get(r.questionId) != null ? ` (${idToScore.get(r.questionId)}đ)` : ""}
            </span>{" "}
      ...
      <p className="mt-6 text-right text-sm font-semibold">Tổng điểm: {sumScores(questions)} / 10</p>
    </div>
  );
}
  ...
  return (
    <div id="print-area">
      <div className="a4-page">
        ...
        <p className="gdt-exam-code">
          Mã đề: {examMeta?.examCode} &nbsp;|&nbsp; Tổng điểm: {sumScores(questions)}
        </p>
