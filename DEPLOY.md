...
| `COLORING_GEMINI_API_KEY` | (dán API key riêng cho tab Tập tô màu) | Dùng khi chỉ có 1 key. **PHẢI khác** key ở `GEMINI_API_KEY` phía trên - tab Tập tô màu dùng model tạo ẢNH, hạn mức free tier tách riêng với tạo văn bản. |
| `COLORING_GEMINI_API_KEYS` | `key1,key2,key3` | Khuyến nghị (giống `GEMINI_API_KEYS`): nhiều key riêng cho tab Tập tô màu, phân tách dấu phẩy. |
