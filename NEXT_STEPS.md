...
| 23 | Khung KHGD Tiểu học: bảng "Kiểm tra, đánh giá định kỳ" riêng | Vẫn CHƯA làm bảng riêng (cần mẫu thật của trường). Phiên 50 đã thêm các DÒNG Ôn tập/Kiểm tra định kì ngay trong bảng chính theo Thông tư 27/2020 Điều 7 - xem #33. |
...
| 26 | ✅ ĐÃ SỬA (Phiên 50): Khung KHGD Tiểu học - "Số tiết/tuần" không còn mặc định cứng 10 | Nay chỉ điền sẵn khi có căn cứ (`TIEU_HOC_TIET_PER_WEEK_DEFAULTS` trong `khgdTieuHocSchedulePolicy.js`: Tiếng Việt Lớp 2 = 10, Lớp 3 = 7, Lớp 5 = 7; Toán Lớp 3 = 5); môn/khối khác để TRỐNG, giáo viên tự nhập (chưa nhập thì cột Tuần/Ghi chú không tự tính). Bảng còn thiếu nhiều môn/khối - xem #34. |
| 27 | Khung KHGD Tiểu học: số tiết mỗi Bài (Toán/Đạo đức/TNXH...) | Phiên 50: Markdown SGK KHÔNG ghi số tiết nên nút **"Tự tính số tiết & xếp Ôn tập/Kiểm tra định kì"** chia ĐỀU phần quỹ tiết học kì còn lại cho các Bài chưa chốt (xem `khgdSchedule.js`). Bài chốt số tiết khi: Tiếng Việt (bản mẫu giáo viên), hoặc tiêu đề Markdown ghi `(N tiết)`, hoặc giáo viên tự sửa. Vẫn chưa có mẫu KHGD Tiểu học của giáo viên cho Toán/Tiếng Anh/Đạo đức/TNXH để biết có cần tách theo hoạt động như Tiếng Việt không (không tự bịa cấu trúc). Chưa kiểm tra Tiếng Việt Lớp 1/3/4/5 và chương 3+ của Lớp 2 (`TIENG_VIET_TIET_RULES` mới đối chiếu Lớp 2 tuần 1–8). |
| 28 | Khung KHGD Tiểu học: dùng AI làm gì thêm ngoài cột "Nội dung điều chỉnh"? | Đang chờ Hoan quyết. Đề xuất: AI chỉ đặt TÊN ngắn cho dòng "Luyện từ và câu" (hiện chỉ ghi chung) và "Viết đoạn văn ..." (hiện là câu dài, VD 100 ký tự), và dùng mô tả hoạt động trong Markdown (`moTa`, parser đã trả sẵn) + "Đọc mở rộng" làm ngữ cảnh cho cột điều chỉnh. Không cho AI thêm/bớt dòng hay đổi số tiết. Chưa làm vì cần test Gemini thật (#20). |
| 30 | ✅ ĐÃ LÀM (Phiên 49b): bộ đọc Markdown **Ngữ văn** riêng - còn 2 việc nhỏ | Hoan gửi file thật Ngữ văn 7 Bài 1 (`# BÀI 1: TÊN` → `## I..VII`). `khgdOutlineService.js` → `buildNguVanRows()` tách theo HOẠT ĐỘNG: Giới thiệu bài học + Tri thức Ngữ văn (gộp 1 dòng), mỗi văn bản đọc hiểu 1 dòng, Thực hành tiếng Việt, Viết, Nói và nghe, Củng cố (bảng Markdown được chuyển thành chữ) = 9 dòng/Bài. **Còn lại:** (a) mới thấy MỘT file (Bài 1) - các Bài 2-4 nếu đặt tiêu đề/phần khác mẫu (VD không có phần III "Đọc hiểu") thì tên dòng vẫn lấy từ tiêu đề nên vẫn chạy, nhưng chưa kiểm chứng; (b) tên dòng do mình suy ra từ tiêu đề Markdown, KHÔNG phải từ PPCT trường Hoan - nếu PPCT thật đặt tên khác (VD "Đọc văn bản 1...") thì chỉnh quy tắc đặt tên trong `buildNguVanRows`. |
| 31 | Khung KHGD Tiểu học: chưa gửi đoạn trích Markdown lên AI (như THCS/THPT đã làm ở Phiên 49) | Nguyên tắc "LUÔN ưu tiên nội dung Markdown" mới áp dụng cho THCS/THPT (Hoan test tab đó). Tiểu học: parser Tiếng Việt đã trả sẵn `moTa` từng hoạt động nhưng `khgdTieuHocPromptTemplates.js` chưa dùng - làm cùng lúc với #28. |
| 32 | Khung KHGD THCS/THPT: khối Tiếng Anh cả năm cần thêm dòng Review/Kiểm tra bằng tay | Markdown mỗi Unit chỉ cho 7 dòng; các dòng Review 1-4/Kiểm tra giữa-cuối kỳ (không thuộc Unit nào) giáo viên tự thêm vào bảng. Trần bài/lượt nay 120 (`KHGD_MAX_LESSONS`), AI chia lô 40 bài/lần (`KHGD_AI_BATCH_SIZE`). |
| 29 | Dữ liệu: lỗi chính tả trong tiêu đề Bài ở Markdown kho GitHub kiến thức | `Tieng_Viet` Lớp 2 `chuong_2.md` dòng 130 ghi `BÀI 11: CÁI TRỐNG TRƯỆNG EM` (đúng: TRƯỜNG); `Tu_Nhien_Xa_Hoi` Lớp 2 `chuong_1.md` dòng 48 ghi `BÀI 2: NGHỀ NGHỆP CỦA NGHƯỜI LỚN ...` (đúng: NGHỀ NGHIỆP CỦA NGƯỜI LỚN). Parser chép nguyên tên trong Markdown nên lỗi sẽ hiện ra bảng; tên trong `chuong_{n}_bai.json` (nếu có) được ưu tiên. Hoan sửa lại trong kho. |
| 33 | Phiên 50: Ôn tập/Kiểm tra định kì + tự tính số tiết (CẢ 2 tab) - CẦN Hoan đối chiếu với Markdown/PPCT thật | **Đã làm:** `khgdSchedule.js` (lõi xếp lịch thuần) + `khgdTieuHocSchedulePolicy.js` (Thông tư 27/2020) + `khgdSchedulePolicy.js` (Thông tư 22/2021) + `khgdSgkReview.js` (nhận diện "Ôn tập/Đánh giá giữa/cuối học kì" TRONG Markdown SGK, ưu tiên hơn đề xuất). **CHƯA làm được:** "kiểm tra hết các khối Tiểu học theo Markdown từng môn" - gói Phiên 49b chỉ có Markdown THCS Lớp 7 (test/fixtures), KHÔNG có file Markdown Tiểu học nào để đối chiếu; parser nhận diện tiêu đề ôn tập mới test bằng Markdown MÔ PHỎNG. Hoan gửi Markdown chương CUỐI mỗi Tập (nơi thường có "Ôn tập và đánh giá giữa/cuối học kì") của Toán, Tiếng Việt, Tiếng Anh, Đạo đức, TNXH, Khoa học, Lịch sử và Địa lí... Lớp 1-5 để Claude đối chiếu tiêu đề thật + thêm `(N tiết)` nếu SGK ghi. |
| 34 | Phiên 50: bảng "Số tiết/tuần gợi ý" mới chỉ có vài môn có căn cứ | Tiểu học: Tiếng Việt L2/L3/L5, Toán L3. THCS/THPT: Ngữ văn, Toán, Tiếng Anh, KHTN (THCS 4/4/3/4), Lịch sử và Địa lí (THCS 3), GDTC/Địa lí/Công nghệ (THPT 2). Các môn/khối còn lại để trống → giáo viên tự nhập (không tự bịa số). Hoan gửi bảng số tiết/năm theo Thông tư 32/2018 (sửa đổi TT13/2022) của trường để điền đủ. |
| 35 | Phiên 50: 2 quy tắc CHƯA có văn bản quy định, chỉ là mặc định gợi ý | (a) Số tiết ÔN TẬP: khi còn bài chưa chốt tiết = ½ tuần (giữa kì) / 1 tuần (cuối kì); khi mọi bài đã chốt (Tiếng Việt L2, Tiếng Anh) = toàn bộ quỹ tiết còn lại chia cho 2 mốc. (b) Bài kiểm tra Tiểu học mặc định 1 tiết. Vị trí giữa kì = sau 8 tuần học (Tuần 9 / Tuần 27) theo PPCT thực tế. Giáo viên sửa được mọi số; Hoan cho biết nếu PPCT trường dùng quy tắc khác. |
| 36 | Phiên 50: chưa thử trên trình duyệt thật + chưa test Gemini thật | Đã có: 29 test mới (kể cả xuất Word thật cho cả 2 tab), build sạch, LibreOffice mở được cả 2 file Word. CHƯA: bấm thử nút mới trên giao diện (viền vàng dòng đề xuất, cảnh báo thiếu chương, chạy lại nhiều lần). Cần Hoan test thật Tiếng Việt Lớp 2 Tập 1 (mong đợi: 160 tiết bài + ôn tập giữa kì Tuần 9 + ôn tập/kiểm tra cuối kì Tuần 17-18 = 180 tiết) và Toán/Ngữ văn Lớp 7. |
| 37 | ✅ ĐÃ SỬA (Phiên 50b): cột Tuần/Ghi chú bị cắt chữ ("Tuần 17-1", "170-17") | Cột bảng quá hẹp cho khoảng dài do "Tự tính số tiết" sinh ra - đã nới rộng cả 2 tab. |
| 38 | ✅ ĐÃ SỬA (Phiên 50b): "JSON.parse: unexpected character..." khi "Tạo Khung KHGD Tiểu học" với danh sách dài | `khgdTieuHocEngine.js` trước đó KHÔNG chia lô (gửi hết ~160-170 dòng 1 lần) - nay chia lô 60 bài/lần như THCS/THPT; `apiClient.js` cũng báo lỗi dễ hiểu hơn khi server trả JSON hỏng. Hoan test lại Tiếng Việt Lớp 2 Tập 1 đầy đủ với Gemini key thật để xác nhận hết lỗi. |
