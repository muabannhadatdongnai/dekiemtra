...
| 30 | Khung KHGD THCS/THPT: chưa có mẫu Markdown **Ngữ văn** để kiểm tra bộ đọc | Phiên 49: Hoan gửi 3 file mẫu Lớp 7 nhưng cả 3 cùng tên `chuong_1.md` nên ghi đè nhau, chỉ nhận được Tiếng Anh + Toán. Ngữ văn (và mọi môn khác ngoài Tiếng Anh/Toán) hiện đi qua bộ đọc "Bài" CHUNG (`## Bài k: Tên` → 1 dòng/Bài, kèm trích nội dung) - đã test bằng Markdown tự dựng, CHƯA bằng file thật. Nếu Ngữ văn cần tách nhiều dòng/Bài (Đọc hiểu văn bản 1/2, Thực hành tiếng Việt, Viết, Nói và nghe...) thì gửi lại file (đổi tên khác nhau, VD `ngu_van_7_bai_1.md`) để làm bộ đọc riêng trong `OUTLINE_BUILDERS` (`khgdOutlineService.js`). |
