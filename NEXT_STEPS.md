...
| 27 | Khung KHGD Tiểu học: các môn KHÁC Tiếng Việt chưa có bộ đọc Markdown | Phiên 48b: chỉ Tiếng Việt Lớp 2 (KNTT) có `khgdTieuHocTiengVietParser.js`. Môn khác: form chỉ lấy được Chủ đề từ Markdown (nếu tiêu đề dạng "CHỦ ĐỀ n: ...") rồi dùng `_bai.json` như cũ. Mỗi môn/định dạng cần 1 parser riêng — cần Hoan gửi Markdown mẫu của môn kế tiếp (VD Toán Lớp 2). Cũng chưa kiểm tra Tiếng Việt Lớp 1/3/4/5 và chương 3+ của Lớp 2 (số tiết/tuần và quy tắc `TIENG_VIET_TIET_RULES` mới đối chiếu Lớp 2 tuần 1–8). |
...
| 29 | Dữ liệu: `chuong_2.md` dòng 130 ghi `BÀI 11: CÁI TRỐNG TRƯỆNG EM` | Sai chính tả ("TRƯỜNG") ở file Markdown trong kho GitHub kiến thức, không phải lỗi code — tên bài trong `chuong_2_bai.json` (nếu có) được ưu tiên nên không ảnh hưởng khi có file đó. Hoan sửa lại trong kho. |
| 30 | Khung KHGD (THCS/THPT): `khgdExportService.js` vẫn đặt `tableHeader: true` | Bảng Word THCS/THPT vẫn lặp hàng tiêu đề ở mỗi trang (Phiên 48b chỉ bỏ ở Tiểu học theo phản hồi của Hoan). Hỏi Hoan có muốn bỏ luôn không. |
